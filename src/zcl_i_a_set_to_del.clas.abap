CLASS zcl_i_a_set_to_del DEFINITION
  PUBLIC
...
CLASS zcl_i_a_set_to_del IMPLEMENTATION.
  ...
  METHOD /bobf/if_frw_action~execute.
    ...
    LOOP AT lt_sales_order ASSIGNING FIELD-SYMBOL(<s_sales_order>).
      ...
      IF <s_sales_order>-delivery_status = ' ' .
        <s_sales_order>-delivery_status  = 'D'.  " PAIDs

      ENDIF.
      IF <s_sales_order>-billing_status EQ 'P' AND <s_sales_order>-delivery_status EQ 'D'.
        <s_sales_order>-status = 'C'."Completed
      ELSEIF  <s_sales_order>-billing_status EQ ' ' AND <s_sales_order>-delivery_status EQ 'D'.
      ...
      io_modify->update(
        ...
        it_changed_fields     = VALUE #( ( zif_i_salesorder_header_c=>sc_node_attribute-zi_salesorder_header-status )
                                         ( zif_i_salesorder_header_c=>sc_node_attribute-zi_salesorder_header-delivery_status ) )
        ).
    ENDLOOP.
  ENDMETHOD.
ENDCLASS.
